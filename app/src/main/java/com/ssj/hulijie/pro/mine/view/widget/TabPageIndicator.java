package com.ssj.hulijie.pro.mine.view.widget;
...
//    = R.drawable.background_tab;//字体颜色的选择器
    ...
    public TabPageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        lineLongerDim = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, lineLongerDim, dm);
...
//        isSameLine = a.getBoolean(R.styleable.PagerSlidingTab_sameLine, isSameLine);
    ...
    private void addTextTab(final int position, String title) {
        ...
        if (currentIndicatorMode == IndicatorMode.MODE_WEIGHT_NOEXPAND_NOSAME_LINE_LONGER) {
            tab.setPadding(lineLongerDim, 0, lineLongerDim, 0);
        }
    ...
    public void scrollToChild(int position, int offset) {
        if (position != 5) {  //修复第一个Item会往前跑的bug  最右边tab
...
//            AppLog.Log("position:" + position + ",positionOffset:" + positionOffset + ",positionOffsetPixels:" + positionOffsetPixels);
...
//            AppLog.Log("onPageScrollStateChanged:" + state);
...
//            AppLog.Log("onPageSelected:" + position);
            if (delegatePageListener != null) {
    ...
    public int getLineLongerDim() {
        return lineLongerDim;
    }

    public void setLineLongerDim(int lineLongerDim) {
        this.lineLongerDim = lineLongerDim;
        invalidate();
    }
